...
class FedBuffServer:
    ...
    def start(self, STOP_EVENT, SELECTED_EVENT, GLOBAL_QUEUE, GLOBAL_INFO):  # start the whole training priod
        ...
        schedule.every(self.global_config["schedule_period"]).seconds.do(self.select, SELECTED_EVENT)
        while self.current_epoch < self.total_epoch:
            self.update(GLOBAL_QUEUE,GLOBAL_INFO)
    ...
    def update(self, GLOBAL_QUEUE, GLOBAL_INFO):
        if self.current_epoch >= self.total_epoch:  # if global training is going on
            return

        if GLOBAL_QUEUE.qsize() >= self.global_config["updater"]["params"]["K"]:  # if server has received some gradients from clients
    ...
    def select(self,SELECTED_EVENT):
        participating_client_idxs = self.schedule(self.global_manager.clients_dict, self.schedule_config,SELECTED_EVENT)
    ...
    def start_select_clients(self, participating_clients_idxs, SELECTED_EVENT):
